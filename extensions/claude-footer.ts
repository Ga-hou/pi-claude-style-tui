...
export function applyClaudeFooter(pi: ExtensionAPI, ctx: ExtensionContext, items: readonly StatusLineItemId[]): void {
	ctx.ui.setFooter((tui, theme, footerData) => {
		...
		return {
			...
			render(width: number): string[] {
				const statusLine = renderStatusLine(items, ctx, theme, footerData.getGitBranch(), pi.getThinkingLevel());
