...
- `/use-default-tui` can restore the theme, header, footer, editor, spinner, and hidden-thinking label. Pi has no API to unregister a tool override during a session, so compact tool renderers remain active for that runtime. `/reload` reloads this package and registers them again; restoring built-in tool shells requires starting Pi without this extension.
